...
# PointNetSetAbstraction类实现普通的Set Abstraction:#首先通过sample_and_group的操作形成局部group,
...
class PointNetFeaturePropagation(nn.Module):
    # Feature Propagation的实现主要通过线性差值和MLP完成。#当点的个数只有一个的时候，采用repeat直接复制成N个点;
    # #当点的个数大于一个的时候，采用线性差值的方式进行上采样，
    # 拼接上下采样对应点的SA层的特征，再对拼接后的每一个点都做一个MLP。
    def __init__(self, in_channel, mlp): #例如in channel=384, mlp=[256,128]
...
if __name__=="__main__":
    model = PointNetSetAbstraction(npoint=512, radius=0.2, nsample=32, in_channel=6, mlp=[64, 64, 128], group_all=False)
    xyz = torch.rand((5,3,10000))
    points = torch.rand((5,3,10000))
    # print(points.shape)
